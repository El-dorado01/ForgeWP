...
export default function VerifyEmailViewWrapper() {
  const { verifyEmail, logout, error, initializing, resendVerificationEmail } =
    useWpAuth();
  ...
  return (
    <Card>
      ...
      <CardContent className='px-8 py-4 text-center text-sm text-zinc-600'>
        {status === 'pending' && (
          <div className='flex flex-col gap-4 text-center'>
            <p className='leading-relaxed'>
              Thank you for signing up! Please check your email inbox and click
              on the verification link to activate your account. Once verified,
            ...
            <div className='border-t border-slate-100 pt-4 mt-2'>
              {resendStatus === 'success' ? (
                <div className='bg-green-50 text-green-700 border border-green-200 text-xs p-3 rounded-md font-medium text-left'>
                  Verification email successfully resent to{' '}
                ...
                <div className='flex flex-col gap-3'>
                  {!showResendForm && resendEmail ? (
                    ...
                    <form
                      ...
                      className='flex flex-col gap-2 max-w-sm mx-auto w-full text-left'
                    >
                      <p className='text-xs text-zinc-500 text-center mb-1'>
                        Enter your email address to request a new verification
                        link.
                      </p>
        ...
        {status === 'verifying' && (
          <p className='leading-relaxed'>
            Communicating with the secure backend server. This process should
            take only a moment.
          </p>
        )}
