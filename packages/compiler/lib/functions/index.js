...
/**
 * Helper to resolve a page's URL by its ForgeWP page-template slug, mirroring the
 * lookup used for page-level useWpPageLink() calls (matches against the
 * "_wp_page_template" post meta assigned to page-[slug].php). Used by Gutenberg
 * block components, which reference useWpPageLink() as plain source text and can't
 * go through the page-level token/regex substitution pipeline.
 */
function forgewp_resolve_page_link($slug, $fallback = '') {
    $current_lang = function_exists('pll_current_language') ? pll_current_language() : '';
    $get_pages_args = array('meta_key' => '_wp_page_template', 'meta_value' => "page-{$slug}.php", 'number' => 1);
    if (!empty($current_lang)) {
        $get_pages_args['lang'] = $current_lang;
    }
    $matched_pages = get_pages($get_pages_args);
    return !empty($matched_pages) ? get_permalink($matched_pages[0]->ID) : $fallback;
}
