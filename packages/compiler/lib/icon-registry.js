/**
 * Curated ForgeWP icon registry (Lucide-compatible slugs → inline SVG).
 * Used by PHP forgewp_render_theme_icon, block editor picker, and WpIcon resolve.
 */

/** @type {Record<string, string>} */
export const FORGEWP_ICON_SVGS = {
  award:
  ...
  hotel:
    '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-hotel"><path d="M10 22v-6.57"></path><path d="M12 11h.01"></path><path d="M12 7h.01"></path><path d="M14 15.43V22"></path><path d="M15 16a5 5 0 0 0-6 0"></path><path d="M16 11h.01"></path><path d="M16 7h.01"></path><path d="M8 11h.01"></path><path d="M8 7h.01"></path><rect x="4" y="2" width="16" height="20" rx="2"></rect></svg>',
  sparkles:
...
export const FORGEWP_DEFAULT_ICON_SLUGS = Object.keys(FORGEWP_ICON_SVGS);

/**
 * Apply class to an SVG root tag (or return as-is).
 */
export function applyIconClass(svg, className) {
  if (!svg) return '';
  if (!className) return svg;
  if (/\sclass="/.test(svg)) {
    return svg.replace(/\sclass="([^"]*)"/, (_, existing) => ` class="${existing} ${className}"`);
  }
  return svg.replace('<svg ', `<svg class="${className}" `);
}

/**
 * PHP array literal for forgewp_render_theme_icon body.
 */
export function buildPhpIconSvgMapLiteral() {
  const lines = Object.entries(FORGEWP_ICON_SVGS).map(([slug, svg]) => {
...
/**
 * JSON object for window.forgeWpIconRegistry in the block editor.
 */
export function buildEditorIconRegistry() {
  return { ...FORGEWP_ICON_SVGS };
}
